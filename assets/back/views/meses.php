	...
	function meses(){
		...
		<div class="body">
			<h1>meses</h1>
		<?php
		$query = "SELECT * FROM `meses` ORDER BY `meses`.`year` ASC;";
		$exe_query = $con->query($query);
		$i = '';
		$j =1;
		while ($mes=$exe_query->fetch_assoc()) {
			?>
				<h2 class="mes"><a class="mes" href="index.php?page=meses&mes=<?=$mes['mes']?>&year=<?=$mes['year']?>"><?=$mes['year']?> - <?=$mes['mes']?></a> | <a href="assets/back/dbconect/querys.php?query=6&cd=<?=$mes['id']?>&mes=<?=$mes['mes']?>&year=<?=$mes['year']?>" class="del-mes" title="">Eliminar</a></h2>
			<?php
		}
		...
		?>
			...
			<form id="form-mes" role="form">
				<div class="form-div">
					...
					<input type="text" class="form-input" name="year">
				</div>
				<div class="form-div">
					<div class="label">
						<span>Mes</span>
	...
	function tabla_mes(){
		...
		<div class="body">
			<h1><a class="mes" href="?page=meses"><i class="fas fa-arrow-circle-left"></i></a> <?=$year?> <?=$mes?></h1>
			<!------------------------------------- ventas --------------------------------->
			...
			<table class="table">
				...
				<tbody>
			<?php
				$query = "SELECT * FROM `Ventas $year - $mes` ORDER BY `Ventas $year - $mes`.`id` ASC;";
				$exe_query = $con->query($query);
				$i = 0;
				while ($row=$exe_query->fetch_assoc()) {
					?>
						<tr class="fila<?=$i?>">
							...
							<td><?=$row['fecha']?></td>
							<td><?=$row['neto']?></td>
							<td><a href="assets/back/dbconect/querys.php?query=5&cd=<?=$row['id']?>&year=<?=$year?>&month=<?=$mes?>" class="btn-delete" id="del">Eliminar</a></td>
						</tr>
					<?php
					if ($i == 0) {
						$i = 1;
			...
			<form id="form-venta" method="post" action="assets/back/dbconect/querys.php?query=4&year=<?=$year?>&month=<?=$mes?>" role="form">
				<div class="form-div">
					<div class="label">
						<span>producto</span>
					</div>
			...
			<table class="table">
				...
				<tbody>
			<?php
				$query = "SELECT * FROM `gastos $year - $mes` ORDER BY `gastos $year - $mes`.`id` ASC;";
				$exe_query = $con->query($query);
				$i = 0;
				while ($row=$exe_query->fetch_assoc()) {
					?>
						...
						<td><?=$row['fecha']?></td>
						<td><?=$row['neto']?></td>
						<td><a href="assets/back/dbconect/querys.php?query=8&cd=<?=$row['id']?>&year=<?=$year?>&month=<?=$mes?>" class="btn-delete" id="del">Eliminar</a></td>
						</tr>
					<?php
					if ($i == 0) {
						$i = 1;
			...
			<form id="form-gastos" method="post" action="assets/back/dbconect/querys.php?query=7&year=<?=$year?>&month=<?=$mes?>" role="form">
				<div class="form-div">
					<div class="label">
						<span>producto</span>
					</div>
			...
			<table class="table">
				...
				<tbody>
			<?php
				$query = "SELECT * FROM `ventas $year - $mes`;";
				$exe_query = $con->query($query);

				$query2 = "SELECT * FROM `gastos $year - $mes`;";
			...
			?>
					<tr class="fila<?=$i?>">
						...
						<td><?=date('d/m/Y')?></td>
						<td><?=$total?></td>
						<td><a class="btn-send" style="margin: auto; text-decoration: none;" href="assets/back/dbconect/querys.php?query=9&year=<?=$year?>&month=<?=$mes?>">añadir al libro</a></td>
					</tr>
				</tbody>
			</table>
		</div>
